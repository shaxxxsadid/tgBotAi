...
import { gigaChat } from './API/gigachat.js';
...
const keyboard = Markup.keyboard([
    [//{ text: '/Ask - Начать работу c Chat GPT' }, 
        { text: '/GIGAChat - Начать работу c GigaChat' }],
...
const inlineKeyboard = Markup.inlineKeyboard([
    ...
    { text: "Начать работу c GIGAChat", callback_data: "GigaChat" }],
...
const whitelist = [812466464, 2039520204, 859006425, 1580855418, 960024617, 527760957]// Список пользователей
...
bot.action('GigaChat', async (ctx) => {
    templateCommand(ctx, 'Жду вашего голосового или текствого сообщения', 5, 1, INITIAL_SESSION)
})
...
bot.command('GigaChat', async (ctx) => {
    await ctx.reply('Жду вашего текствого сообщения');
    jobs = 5;
})
...
const GigaChat = async (ctx, text) => {
    ...
    try {
        await ctx.reply(code('Сообщение принято. Ожидайте...'))
        session[chatId] = session[chatId] || []
        session[chatId].push({ role: "assistant", content: text })
        
        const response = await gigaChat.chat(session[chatId])
        response != null ? response : () => {
            session[chatId] = session[chatId] || []
            response = "Увы ошибка..."
        }
        await ctx.reply(response)
        console.log(JSON.stringify('User: ' + ctx.message.chat.username + ' Вопрос к GigaChat: ' + text, null, 2) + '\n Ответ: ' + response)
    } catch (e) {
        console.log("Error while message with GIGAChat", e.message)
    }
}
    ...
    try {
        ...
        switch (jobsWithGPT) {
            ...
            case 1: // Work with ChatGPT
                ...
                ctx.reply(code('Ваш запрос принят, но нет возможности отвечать на него. ChatGPT прекратил работу в РФ.'));
                // await ctx.reply(code(`Текст вашего запроса: ${text}`))
                // await ChatGPTChat(ctx, text)
                break;
            case 2: // Generate image whith openai
                await ctx.reply(code('Ваш запрос принят, но нет возможности отвечать на него. ChatGPT прекратил работу в РФ.'));  
                // await ctx.reply(code(`Текст вашего запроса: ${text}`))
                // await ChatGPTChat(ctx, text)
                break;
        }
...
bot.on(message('text'), async ctx => {
    ...
    switch (jobs) {
        ...
        case 4: // Generate image whith openai
            ...
            //ChatGPTChat(ctx, ctx.message.text);
            break;
        case 5: // Chatnig with GIGAChat
            GigaChat(ctx, ctx.message.text);
            break;
    }
})
